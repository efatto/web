...
class Binary(openerpweb.Controller):
    ...
    def saveas_ajax(self, req, data, token):
        ...
        filename = jdata.get('filename', None)
...
class Reports(openerpweb.Controller):
    ...
    def index(self, req, action, token):
        ...
        # 20150811 - PRINT SINGLE REPORT WITH ATTACHMENT FILE NAME
        # use the attachment name only if the report refers to a single id
        attachment = action.get('attachment')
        if attachment and len(report_ids) == 1:
            import openerp
            import pooler
            dbname = None
            uid = None
            ...
            registry = openerp.modules.registry.Registry(dbname)
            with registry.cursor() as cr:
                obj = pool.get(action.get('model')).browse(cr, uid,
                    report_ids[0], context)
                tmp_file_name = eval(attachment, {'object': obj, 'time': time})
                if tmp_file_name:
